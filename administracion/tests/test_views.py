...
class AdministracionViewsTest(TestCase):
    ...
    def test_gestion_categorias_view(self):
        self.client.login(username='@admin', password='password')
        response = self.client.get(reverse('administracion:gestion_categorias'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'administracion/gestion_categorias.html')
    ...
    def test_asignar_roles_usuario_view(self):
        rol_prueba = Rol.objects.create(nombre='administrador')
        self.client.login(username='@testuser', password='testpassword')
